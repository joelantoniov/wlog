...
char* Shell::version(){
    ...
    sprintf(v, "%sv - %s",fversion,date);
    return v;
}

void Shell::banner() {
    _message.print(NORMAL, "%s %s Author - (%s)", name, version(), author);
}
...
bool Shell::isInteger(char *s) {
    ...
    int i = 0;
    if (s[i] == '+' || s[i] == '-') ++i;
    while (i < strlen(s) && isInt) isInt = isInt && isdigit(s[i++]);
    return isInt;
}
...
int Shell::getSwitch(char *s) {
    ...
    if (strcmp(s, "+") == 0 || strcmp(s, "1") == 0 || strcmp(s, "on") == 0 || strcmp(s, "true") == 0) return 1;
    else if (strcmp(s, "-") == 0 || strcmp(s, "0") == 0 || strcmp(s, "off") == 0 || strcmp(s, "false") == 0) return 0;
    else return -1;
}
