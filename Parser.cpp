    ...
    while( synchronized(translationUnitFirstSet, followSet, "Expected TranslationUnit") ) {
        ...
        if(_lookAhead.getTokenType() == SYM_SQ_OPEN ||
            ...
            if(_lookAhead.getTokenType() == SYM_SQ_OPEN ){ 
                match(SYM_SQ_OPEN);
                match(LIT_INT);
                if(_lookAhead.getTokenType() == SYM_SQ_CLOSE) match(SYM_SQ_CLOSE); 
                if(type != "v") _symbolTable->reDefine(identifier, type == "i" ? "I" : "F");
            }
            ...
            if(_lookAhead.getTokenType() == SYM_SEMICOLON ) match(SYM_SEMICOLON);
            else {
                if(externDef) {
...
void Parser::CompoundStatement(string functionName) {
    ...
    static tokenType statementFirstSet[] = {SYM_PLUS, SYM_MINUS, SYM_NOT, SYM_CURLY_OPEN, KW_IF, KW_WHILE, KW_FOR, KW_RETURN, SYM_OPEN, LIT_INT, LIT_FLOAT, LIT_STR, TOK_IDENT, (tokenType) - 1};
    static tokenType followSet[] = {TOK_EOF, KW_ELSE, SYM_CURLY_CLOSE, KW_VOID, KW_INT, KW_FLOAT, SYM_PLUS, SYM_MINUS, SYM_NOT, SYM_CURLY_OPEN, KW_IF, KW_WHILE, KW_FOR, KW_RETURN, SYM_OPEN, LIT_INT, LIT_FLOAT, LIT_STR, TOK_IDENT, (tokenType) - 1};
...
void Parser::Statement(string functionName) {
    ...
    static tokenType statementFirstSet[] = {SYM_PLUS, SYM_MINUS, SYM_NOT, SYM_CURLY_OPEN, KW_IF, KW_WHILE, KW_FOR, KW_RETURN, SYM_OPEN, LIT_INT, LIT_FLOAT, LIT_STR, TOK_IDENT, SYM_SEMICOLON, (tokenType) - 1};
    static tokenType followSet[] = {TOK_EOF, KW_ELSE, SYM_CURLY_CLOSE, KW_VOID, KW_INT, KW_FLOAT, SYM_PLUS, SYM_MINUS, SYM_NOT, SYM_CURLY_OPEN, KW_IF, KW_FOR, KW_WHILE, KW_RETURN, SYM_OPEN, LIT_INT, LIT_FLOAT, LIT_STR, TOK_IDENT, (tokenType) - 1};
    ...
    if(synchronized(statementFirstSet, followSet, "Expected Statement") ) {
        if(_lookAhead.getTokenType() == SYM_CURLY_OPEN ) {
            ...
            _symbolTable->closeScope();
        } else if(_lookAhead.getTokenType() == KW_IF ) SelectionStatement(functionName);
          else if(_lookAhead.getTokenType() == KW_WHILE ) RepetitionStatement(functionName);
          else if(_lookAhead.getTokenType() == KW_FOR ) RepetitionStatement2(functionName);
          else if(_lookAhead.getTokenType() == KW_RETURN ) ReturnStatement(functionName);
...
void Parser::RepetitionStatement(string functionName) {
    ...
    static tokenType firstSet[] = {KW_WHILE, KW_FOR, (tokenType) - 1};
    static tokenType followSet[] = {TOK_EOF, KW_ELSE, SYM_CURLY_CLOSE, KW_VOID, KW_INT, KW_FLOAT, SYM_PLUS, SYM_MINUS, SYM_NOT, SYM_CURLY_OPEN, KW_IF, KW_WHILE, KW_FOR, KW_RETURN, SYM_OPEN, LIT_INT, LIT_FLOAT, LIT_STR, TOK_IDENT, (tokenType) - 1};
...
void Parser::RepetitionStatement2(string functionName) {
    _message.print(DBUG, "PARSER: In RepetitionStatement()\n");
    
    static tokenType firstSet[] = {KW_FOR, (tokenType) - 1};
    static tokenType followSet[] = {TOK_EOF, KW_ELSE, SYM_CURLY_CLOSE, KW_VOID, KW_INT, KW_FLOAT, SYM_PLUS, SYM_MINUS, SYM_NOT, SYM_CURLY_OPEN, KW_IF, KW_WHILE, KW_FOR, KW_RETURN, SYM_OPEN, LIT_INT, LIT_FLOAT, LIT_STR, TOK_IDENT, (tokenType) - 1};
    ...
    if(synchronized(firstSet, followSet, "Expected Repetition Statement") ) {
        ...
        match(SYM_CLOSE);*/
        std::cout << "HERE WE AREEEE" << std::endl;
        Statement(functionName);
    }
    _message.print(DBUG, "PARSER: End of RepetitionStatement()\n");
}
...
void Parser::ReturnStatement(string functionName) {
    ...
    if(synchronized(firstSet, followSet, "Expected Return Statement") ) {
        ...
        if(memberOf(_lookAhead.getTokenType(),expressionFirstSet) ) type = Expression();
...
string Parser::RelationExpression() {
    ...
    if(synchronized(firstSet, followSet, "Expected Relational Expression") ) {
        ...
        if(memberOf(_lookAhead.getTokenType(), set) ) {
            ...
            if(_lookAhead.getTokenType() == SYM_LESS_EQ) match(SYM_LESS_EQ);
            else if(_lookAhead.getTokenType() == SYM_LESS)  match(SYM_LESS);
            else if(_lookAhead.getTokenType() == SYM_GREATER_EQ) match(SYM_GREATER_EQ);
            else if(_lookAhead.getTokenType() == SYM_GREATER) match(SYM_GREATER);
            else if(_lookAhead.getTokenType() == SYM_EQUAL) match(SYM_EQUAL);
            else if(_lookAhead.getTokenType() == SYM_NOT_EQ) match(SYM_NOT_EQ);
    ...
    if(synchronized(firstSet, followSet, "Expected Value") ) {
        if(_lookAhead.getTokenType() == SYM_OPEN ) {
            ...
            if(!_symbolTable->isDefined(_lookAhead.getLexeme())){
              string tmp = identifier.c_str();
              if(tmp == "for"){save = "for";}else{
            ...
            if(_lookAhead.getTokenType() == SYM_SQ_OPEN ) {
                ...
                if(!type.empty()) type = type.substr(0,1);
                match(SYM_OPEN);
